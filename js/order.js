...
// let summary = document.querySelector(".submit")
document.getElementById("checkout-form").addEventListener("submit", function(event) {
  event.preventDefault();
  
  let name = document.querySelector("#naming").value;
  let phone = document.querySelector("#phoneno").value;
  let email = document.querySelector("#email").value;
  let address = document.querySelector("#add").value;
  let payment = document.querySelector('input[name="payment"]:checked').value;

  if (name && phone && email && address && payment) { // to only run this if all the value is correct
    document.querySelector("#cart-display").style.display = "none";
    let rec = document.querySelector("#cart-summary")
    console.log(rec)
    cart1.forEach(product => {
      let color = product.Color
      let name = product.Name
      ...
      let UId=product.UID
      let pc=product.PRODUCTCODE
      rec.innerHTML= `<div class="product1" data-id="${id}" data-pc="${pc}" data-size="${size}" data-quantity="${quantity}" data-price="${price}"  data-uid="${UId}">
      <div id="encloser1">
      <div id="info1">
      <p id="name1">${name}</p>
      <p id="price1">Price:$${price}</p>
